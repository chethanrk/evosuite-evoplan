sap.ui.define([
	...
	return BaseController.extend("com.evorait.evoplan.controller.gantt.GanttResourceFilter", {
		...
		open: function (oView, oTreeTable) {
			...
			if (!this._oDialog) {
				...
				Fragment.load({
					...
					// this._oFilterBar.attachReset(function (oEvent) {
					// 	this.onClear(oEvent);
					// 	this.onCustomFilterChange(oEvent);
					// 	// resolve();
					// }.bind(this));
		...
		onGanttResourceFilterChange: function (oEvent) {
			...
			// var oResourceGuids = this._oFilterBar.getControlByKey("ResourceGroupGuid").getSelectedKeys(),
			// 	oFilters = [];
			// if (oResourceGuids && oResourceGuids.length) {
			// 	oResourceGuids.forEach(function (entry) {
			// 		oFilters.push(new Filter("ResourceGroupGuid", FilterOperator.EQ, entry));
			// 	});
			// 	oFilters = new Filter({
			// 		filters: oFilters,
			// 		and: false
			// 	});
			// }
			// oFilters.push(new Filter("ResourceGroupGuid", FilterOperator.ALL, oResourceGuids));
